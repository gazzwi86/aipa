"""Tests for the time-lookup skill.
...
@pytest.mark.eval
class TestTimeLookupEval:
    """Agent evaluation tests for time lookup skill."""

    def test_agent_can_tell_time(self):
        """Agent should be able to tell the current time."""
        # This is a prompt-based eval - would call agent
        _prompt = "What time is it?"
        expected_criteria = {
            "mentions_time": "regex:\\d{1,2}:\\d{2}",
            "mentions_period": "regex:AM|PM|am|pm",
        }

        # Simulate response using actual time
        result = get_current_time()
        response = f"It's {result['time_12h']} {result['timezone_abbrev']}."

        from tests.evals.test_agent_basics import check_response_criteria

        results = check_response_criteria(response, expected_criteria)
        assert all(results.values()), f"Failed criteria: {results}"

    def test_agent_can_tell_time_in_city(self):
        """Agent should be able to tell time in a specific city."""
        _prompt = "What time is it in Tokyo?"
        expected_criteria = {
            "mentions_time": "regex:\\d{1,2}:\\d{2}",
            "mentions_tokyo_or_jst": "regex:Tokyo|JST|Japan",
        }

        result = get_current_time("Tokyo")
        response = f"In Tokyo, it's currently {result['time_12h']} JST."
    ...
    def test_agent_handles_unknown_city(self):
        """Agent should handle unknown cities gracefully."""
        _prompt = "What time is it in Narnia?"

        result = get_current_time("Narnia")
        response = result["error"] if not result["success"] else "error"

        assert "Unknown timezone" in response
