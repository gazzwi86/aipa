"""Tests for the script-runner skill.
...
@pytest.mark.eval
class TestScriptRunnerEval:
    """Agent evaluation tests for script runner skill."""
    ...
    def test_agent_can_parse_csv(self, temp_workspace):
        """Agent should be able to parse CSV data."""
        # Create test CSV
        csv_file = temp_workspace / "files" / "sales.csv"
        csv_file.write_text("product,price,quantity\nWidget,10,5\nGadget,20,3\n")
...
''',
        ...
        assert result.success is True
        assert "$110" in result.output  # 10*5 + 20*3 = 110
